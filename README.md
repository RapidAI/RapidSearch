...
- `GET /papers` — public HTML list (ZH/EN), filter/search, and download links (light theme). Settings link always visible. Tag filter keys: `self-evolution` (agent自进化), `security` (agent安全), `security-top` (安全顶会), `both` (agent安全自进化), `llm-iot` (LLM based 物联网), `survey` (综述), `llm-training` (LLM 训练), `agent-tools-memory` (agent工具与记忆), `other` (其它). Optional paper field `venue` (human-readable conference name, e.g. `IEEE S&P / Oakland`) is shown as a card chip. The toolbar chip **Has Chinese translation** / **仅显示有中文翻译** (`?zh=1`, kept alongside `venue` / `year`) shows only papers with a completed Chinese PDF. It composes with search text, topic chips, and Big-4 venue/year. No login.
...
- `GET /papers/api?q=&tag=&zh=` — public live JSON catalog from `manifest.json` (`page_count`, `zh_pdf`, `dual_pdf`, `translate_status`, `translate_skip_reason`, `can_manage`, `has_review`, `avg_stars`, `rating_count`). First paint does **not** use this route (it can exceed 2 MiB). `page_count` is computed from the local PDF. Papers with `page_count > 100` get `translate_skip_reason=too_many_pages` and are not translated (100 pages is allowed). `tag=` matches stable English keys. `zh=1` (also `true` / `yes` / `on`) keeps papers whose `zh_pdf` is set. That field is filled only when `pdfs/zh/{id}.zh.pdf` exists, which is the same signal as the public 「中文版」 link. Queued or in-progress jobs without that file are excluded. A re-translate that is still queued keeps the previous `zh_pdf`, so the paper stays in the filtered list. `translate_status=done` is not the filter: done also requires the bilingual PDF and is cleared while a re-translate runs. `q`, `tag`, and `zh` compose. No API keys / translate-config / rater list.
...
- Official Big-4 lists are ingested by `agent-papers/ingest_security_top_official.py` from DBLP TOC HTML (`conf/sp/spYYYY`, `conf/ccs/ccsYYYY`, `conf/uss/ussYYYY`, `conf/ndss/ndssYYYY`, 2022–2026). The script solves the DBLP Anubis challenge, stores venue as `IEEE S&P` / `ACM CCS` / `USENIX Security` / `NDSS`, and writes `official` / `before` / `after` / `cov%` in the ingest report. See `agent-papers/README.md`. A 404 year keeps prior catalog rows. Running the script is an ops step on the papers host.
