...
- Official Big-4 lists (IEEE S&P, ACM CCS, USENIX Security, NDSS, 2022 through the latest DBLP volume) are ingested by `agent-papers/ingest_security_top_official.py` from DBLP main-volume TOC pages (`conf/sp/spYYYY`, `conf/ccs/ccsYYYY`, `conf/uss/ussYYYY`, `conf/ndss/ndssYYYY`). See `agent-papers/README.md` for the venue×year map and the `official_count` / `in_catalog` / `missing` / `pdf_ok` report. Running that script does not update papers.maclaw.top.
