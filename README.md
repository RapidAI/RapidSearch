...
- `POST /papers/import` — public (lightly rate-limited) manual import. JSON `{ "url_or_id": "2504.01990", "tag": "llm-training" }`. Accepts an arXiv id, arXiv abs/pdf URL, or a direct http(s) PDF URL. Downloads the PDF into `PAPERS_DIR/pdfs`, upserts `manifest.json` (and `papers.db` when present) with `source=manual` and the chosen tag, then runs the same catalog / optional `auto_translate` / abstract-ZH path as synced papers.
