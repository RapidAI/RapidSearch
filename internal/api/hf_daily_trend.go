...
func generateHFDailyTrendOpenAI(ctx context.Context, client *http.Client, snap translateSnapshot, date string, papers []paperEntry) (hfDailyTrend, error) {
	...
	body, err := postPapersChat(ctx, client, base, snap.APIKey, payload)
	if err != nil {
		return empty, err
	}
...
func postPapersChat(ctx context.Context, client *http.Client, base, apiKey string, payload []byte) ([]byte, error) {
	urls := []string{strings.TrimRight(base, "/") + "/chat/completions"}
	if !strings.HasSuffix(strings.TrimRight(base, "/"), "/v1") {
		urls = append(urls, strings.TrimRight(base, "/")+"/v1/chat/completions")
	}
	if client == nil {
		client = &http.Client{Timeout: hfDailyTrendHTTPTimeout}
	}
