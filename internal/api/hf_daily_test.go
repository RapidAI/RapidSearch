...
func TestHFDailyServesStaleCacheWithoutBlocking(t *testing.T) {
	h, dir := papersHandler(t)
	srv := h.(*Server)
	ps := srv.papers()
	today := todayUTC(ps.daily.now)
	papers, err := parseHFDailyPapers([]byte(hfDailyFixture), today)
	if err != nil {
		t.Fatal(err)
	}
	rec := hfDailyFile{
	...
	if err := os.MkdirAll(hfDailyDir(dir), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := ps.daily.saveFile(rec); err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		rr = httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/papers/daily/"+today, nil)
		h.ServeHTTP(rr, req)
		close(done)
	}()
	select {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("stale daily GET blocked on upstream refresh")
	}
	if rr.Code != http.StatusOK {
		t.Fatalf("status=%d %s", rr.Code, rr.Body.String())
	}
	...
	if !day.OK || !day.FromCache || !day.Stale || day.Count != 2 {
		t.Fatalf("%+v", day)
	}
	cc := rr.Header().Get("Cache-Control")
	if !strings.Contains(cc, "stale-while-revalidate") {
		t.Fatalf("daily cache headers: %s", cc)
	}
	close(blocked)
...
func TestHFDailyDayGzipAnd304(t *testing.T) {
	h, _ := papersHandler(t)
	srv := h.(*Server)
	ps := srv.papers()
	ps.daily.fetchFn = func(ctx context.Context, date string) ([]byte, error) {
	...
	if rr.Code != http.StatusNotModified {
		t.Fatalf("304 status=%d body=%s", rr.Code, rr.Body.String())
	}
}
